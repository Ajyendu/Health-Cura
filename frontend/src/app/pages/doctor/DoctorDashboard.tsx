...
export default function DoctorDashboard() {
  ...
  const loadAppointments = useCallback(async (silent = false) => {
    try {
      ...
      const mapped = (payload?.data || []).map((appt: any) => ({
        ...
        medicalHistory: (appt.medicalHistorySnapshot || []).map((record: any) => ({
          recordId: String(record.recordId || record._id || ""),
        ...
        status: appt.status === "upcoming" || appt.status === "pending" ? "scheduled" : appt.status,
  ...
  useEffect(() => {
    ...
    doctorsApi
      .getMyProfile()
      .then((payload) => setDoctorName(payload?.data?.name || "Doctor"))
      .catch(() => {});
    ...
    <Card className="p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="bg-blue-50 rounded-lg p-3 mb-4">
        <p className="text-sm font-medium mb-2">
          Medical History Shared: {appointment.medicalHistory.length} record(s)
        </p>
        {appointment.medicalHistory.length ? (
          <div className="space-y-1 max-h-28 overflow-y-auto">
            {appointment.medicalHistory.slice(0, 6).map((record) => (
              <p key={record.recordId} className="text-xs text-gray-700 truncate">
                {record.title} ({record.type})
              </p>
            ))}
          </div>
        ) : (
          <p className="text-xs text-gray-600">No records shared yet.</p>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Welcome back, {withDrPrefix(doctorName)}</h1>
