...
import { Stethoscope, LayoutDashboard, User, LogOut, Bell } from "lucide-react";
...
export function DoctorNav() {
  ...
  useEffect(() => {
    appointmentsApi
      .list({ status: "pending", page: 1, limit: 8 })
          ...
          <div className="flex items-center gap-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" className="relative text-gray-600 hover:text-gray-900">
              ...
              <PopoverContent align="end" className="w-80">
                <p className="font-semibold mb-2">Appointment Notifications</p>
                {notifications.length ? (
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {notifications.map((item) => (
                      <div key={item._id} className="border rounded-md p-2">
                        <p className="text-sm font-medium">
                          New request: {item.patientSnapshot?.name || "Patient"}
                        </p>
